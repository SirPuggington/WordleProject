<!doctype html>
<html lang="de">

<head>
    <meta charset="UTF-8">

    <title>Über Wordle</title>
    <link rel="stylesheet" href="../css/main.css">
    <link rel="stylesheet" href="../css/menu.css">

    <link rel="shortcut icon" type="image/x-icon" href="../favicon2.ico">

</head>

<body>

    <?php include "templates/header.php" ?>
    <?php include "templates/help.php" ?>


    <main class="content-wrapper">

        <article>
            <h1>Wordle</h1>
            <section>
                <h2>Was ist Wordle?</h2>
                <p>

                    <strong>Wordle</strong>
                    ist ein Spiel, welches im Oktober 2021 an Beliebtheit im Internet gewann und
                    heutzutage tausende verschiedene Versionen, wie <abbr title="Löse zwei Wordles gleichzeitig">Dordle</abbr>,
                    <abbr title="Finde das gesuchte Land">Worldle</abbr>,
                    <abbr title="Finde das gesuchte Pokémon">Squirdle</abbr> und
                    auch <abbr title="Finde den Titel eines geschwärzten Wikipedia-Artikels">Redactle</abbr>, besitzt. Das Spiel <em>Wordle</em> wurde von Josh Wardle
                    und seiner Frau erfunden sowie von diesen im Internet verbreitet.
                    Die Idee kam ihnen von einem ähnlichen Rätselspiel der NY-Times der <em>Speeling Bee</em>, wessen Prinzip er für sein Browserspiel übernahm.
                </p>
            </section>
            <section>
                <h2>Von 0 auf 2.000.000 in 2 Monaten </h2>
                <p>
                    Zu Beginn des Spieles gab es nach einem Monat "nur" 90 aktive tägliche Spieler/-innen.
                    Jedoch mit Hilfe des hinzugefügten Features des "Teilen" Buttons, welcher erlaubte
                    ein erfolgreiches Erraten des Wortes an die
                    Öffentlichkeit (Social Media) zu verbreiten, steigte die Spielerzahl rasant an.
                    Sodass Mitte Januar bereits 2 Millionen aktive Spieler/-innen ihren eigenen Versuch starten wollten das tägliche Wordle zu erraten.
                </p>
            </section>
            <section>
                <h2>Happy End?</h2>
                <p>
                    Durch den immer weiter steigenden Erfolg Wordles kam es dazu, dass es schon
                    im selben Januar eine Deutsche Version des Spiels gab.
                    o kam es dazu, dass die NY-Times von der auch die Idee zum Wordle stammt
                    das Spiel für „einen niedrigen siebenstelligen Betrag“ erworben hat.
                </p>
            </section>

            <aside>
                <a href="https://de.wikipedia.org/wiki/Wordle" target="_blank">
                    <table>
                        <tr>
                            <td><img src="https://upload.wikimedia.org/wikipedia/commons/0/07/Wikipedia_logo_%28svg%29.svg" alt="Wikipedia"></td>
                            <td>mehr zu Wordle</td>
                        </tr>
                    </table>
                </a>

            </aside>


            <hr>
            <br>
            <br>
            <figure>
                <video src="../assets/video/wordle.mp4" controls></video>
                <figcaption>
                    Quelle: <a href="https://youtu.be/WnWPXZ6vQB8" target="_blank">Youtube: TODAY</a>
                </figcaption>
            </figure>

            <h2>
                Lust auf noch mehr Wordle?
            </h2>
            <table id="wordle-table">
                <caption>Liste von Wordle-Varianten</caption>
                <thead>
                    <tr>
                        <th>Name</th>
                        <th>Beschreibung</th>
                        <th>Link</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Wordle</td>
                        <td>Klassisches Wordle</td>
                        <td><a href="https://wordle.at/" target="_blank">Wordle</a></td>
                    </tr>
                    <tr>
                        <td>Dordle</td>
                        <td>Zwei Wordles gleichzeitig</td>
                        <td><a href="https://zaratustra.itch.io/dordle" target="_blank">Dordle</a></td>
                    </tr>
                    <tr>
                        <td>Lewdle</td>
                        <td>Wordle mit bösen Wörtern</td>
                        <td><a href="https://www.lewdlegame.com/" target="_blank">Lewdle</a></td>
                    </tr>
                    <tr>
                        <td>Wordle unlimited</td>
                        <td>Unendlich viele Wordles </td>
                        <td><a href="https://wordleunlimited.org/" target="_blank">Wordle unlimited</a></td>
                    </tr>
                    <tr>
                        <td>Quordle</td>
                        <td>Vier Wordles gleichzeitig </td>
                        <td><a href="https://www.quordle.com/#/" target="_blank">Quordle</a></td>
                    </tr>
                    <tr>
                        <td>Centordle</td>
                        <td>100 Wordles gleichzeitig </td>
                        <td><a href="https://hked.live/centordle/" target="_blank">Centordle</a></td>
                    </tr>
                    <tr>
                        <td>Poeltl</td>
                        <td>Wordle mit aktiven Basketballspielern </td>
                        <td><a href="https://poeltl.dunk.town/" target="_blank">Poeltl</a></td>
                    </tr>
                    <tr>
                        <td>Chessle</td>
                        <td>Wordle mit Schach (Startkombinationen erraten) </td>
                        <td><a href="https://dordle.io/chessle" target="_blank">Chessle</a></td>
                    </tr>
                    <tr>
                        <td>Redactle</td>
                        <td>Erraten eines Artikels (Wikipedia) </td>
                        <td><a href="https://www.redactle.com/" target="_blank">Redactle</a></td>
                    </tr>
                    <tr>
                        <td>Worldle</td>
                        <td>Erraten des Landes (Mit Fläche des Landes) </td>
                        <td><a href="https://worldle.teuteuf.fr/" target="_blank">Worldle</a></td>
                    </tr>
                    <tr>
                        <td>Semantle</td>
                        <td>Erraten von Wörtern über Semantische Ähnlichkeit </td>
                        <td><a href="https://semantle.com/" target="_blank">Semantle</a></td>
                    </tr>
                    <tr>
                        <td>Squirdle</td>
                        <td>Erraten von Pokémon (anhand ihrer Eigenschaften) </td>
                        <td><a href="https://squirdle.fireblend.com/" target="_blank">Squirdle</a></td>
                    </tr>
                    <tr>
                        <td>Cardle</td>
                        <td>Erraten von Hearthstone Karten </td>
                        <td><a href="https://www.cardlegame.com/" target="_blank">Cardle</a></td>
                    </tr>
                    <tr>
                        <td>Lordle of the Rings</td>
                        <td>Erraten von Tolkien-Begriffen</td>
                        <td><a href="https://digitaltolkien.github.io/vue-wordle/" target="_blank">Lordle of the Rings</a></td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr>
                        <td>mehr..?</td>
                        <td>Kennen sie noch mehr Varianten?</td>
                        <td><a href="mailto:humanbeeing@live.de">Schreiben Sie uns</a></td>
                    </tr>
                </tfoot>

            </table>

        </article>

    </main>

    <?php include "templates/footer.php" ?>


</body>

</html>